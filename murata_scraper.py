...
def get_page_links(session, url): #ページごとに変更が必要
    ...
    links_dates = []
    section_elements = response.html.find(" #PageBrowserLayout > main > div > div.l-container.l-main__contents > div > section > div.c-news > ul ") #構造に合わせて変更
    for section in section_elements:
        a_elements = section.find(" li > a ") #構造に合わせて変更
        p_elements = section.find(" li > a > div.c-news__info > div.c-news__date ") #構造に合わせて変更
...
if __name__ == "__main__":
    key = 'murata' #keyをcompany_dictのkeyに変更
    company_name = company_dict[key]['name'] #keyを変更
    ignore_strings = ['.pdf'] #リンク先にpdfがある場合、除外する文字列を追加
    params_list = [
        {'url': company_dict[key]['newsUrl'],'is_multi_page': True,'total_page':13 ,'get_page_links_func': get_page_links},
        {'url': company_dict[key]['newsUrl2'], 'is_multi_page': True, 'total_page': 4,'get_page_links_func': get_page_links2}
    ]
