...
def get_page_links(session, url): #ページごとに変更が必要
    ...
    links_dates = []
    section_elements = response.html.find("li.c-news-list__item")
    print(section_elements)
    for section in section_elements:
        a_elements = section.find("a")
        print(a_elements)
        p_elements = section.find("a time")
        print(p_elements)
...
def main(company_name, url, ignore_strings):
    ...
    for page in range(1, 25):
        if page != 1:
            url = f"https://press.jal.co.jp/ja/index_{page}.html"
