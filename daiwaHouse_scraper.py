...
def get_page_links(session, url): #ページごとに変更が必要
    ...
    links_dates = []
    section_elements = response.html.find(" section ") #構造に合わせて変更
    for section in section_elements:
        a_elements = section.find("  div > dl > dd > span.newsTxt.external > a[target='_blank'] ") #構造に合わせて変更
        p_elements = section.find("  div > dl > dt ") #構造に合わせて変更
...
if __name__ == "__main__":
    key = 'daiwaHouse' #keyをcompany_dictのkeyに変更
