...
def get_page_links(session, url): #ページごとに変更が必要
    ...
    links_dates = []
    section_elements = response.html.find("section.section.newsBlock") #構造に合わせて変更
    print(section_elements)
    for section in section_elements:
        a_elements = section.find("td.title ul li a") #構造に合わせて変更
        print(a_elements)
        p_elements = section.find("tr th") #構造に合わせて変更
        print(p_elements)
        for a, p in zip(a_elements, p_elements):
            relative_link = a.attrs.get("href", "")
            absolute_link = urljoin(url, relative_link)
            print(absolute_link)
            p_text = '2023年' + p.text[:-3] #日付の形式に合わせて変更
            date = utils.convert_date_format(p_text, '%Y年%m月%d日') #日付の形式に合わせて変更
            print(date)
...
if __name__ == "__main__":
    key = 'omron' #keyをcompany_dictのkeyに変更
