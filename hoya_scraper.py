...
def get_page_links(session, url): #ページごとに変更が必要
    ...
    try:    
        ...
        links_dates = []
        section_elements = response.html.find(" body > main > section > div > ul.news-list.eg-parent ") #構造に合わせて変更
        for section in section_elements:
            a_elements = section.find(" li > p.item-title > a ") #構造に合わせて変更
            p_elements = section.find(" li > time ") #構造に合わせて変更
...
if __name__ == "__main__":
    key = 'hoya' #keyをcompany_dictのkeyに変更
